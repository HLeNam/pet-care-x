 ...
 */
function withStaffGuard<P>(Component: React.ComponentType<P>, redirectTo: string = '/login') {
  const GuardedComponent: React.FC<React.PropsWithChildren<P>> = (props: React.PropsWithChildren<P>) => {
    ...
    const isAccessibleToDoctor =
      (isAuthenticated && profile?.roles.includes('ROLE_STAFF')) || profile?.roles.includes('ROLE_ADMIN');
  ...
  GuardedComponent.displayName = `WithStaffGuard(${Component.displayName ?? Component.name ?? 'Component'})`;
  return GuardedComponent;
}

export default withStaffGuard;
